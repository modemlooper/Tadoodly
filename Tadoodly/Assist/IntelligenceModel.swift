...
final class ProjectPlanner {
    ...
    func generate(for project: Project) -> AsyncThrowingStream<Planner.PartiallyGenerated, Error> {
        AsyncThrowingStream { continuation in
            Task {
                do {
                    let prompt = "I want to create a project. Use this to formulate a response: '\(project.name)' with the following description: \(String(describing: project.projectDescription)). Please generate a task list."
